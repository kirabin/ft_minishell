...
/*   Updated: 2021/04/16 15:03:09 by msamual          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		redirect(t_raw_command *com, char *file_name)
{
	if (com->redirect == 3)
	{
		...
		com->redirect_out = open(file_name, O_APPEND | O_CREAT, S_IRWXO);
		if (com->redirect_out == -1)
	...
	else if (com->redirect == 2)
	{
		if (com->redirect_out != -1)
			close(com->redirect_out);
		com->redirect_out = open(file_name, O_RDONLY | O_CREAT, S_IRWXO);
		if (com->redirect_out == -1)
			return (puterror(NULL, errno));
	}
	else if (com->redirect == 1)
	{
		...
		com->redirect_in = open(file_name, O_WRONLY, S_IRWXO);
		if (com->redirect_in == -1)
			return (puterror(NULL, errno));
	}
	return (0);
